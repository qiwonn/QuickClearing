...
public class ParticleSystem {
	...
	private Runnable endOnceCallback=null;
	
	public ParticleSystem setEndOnceCallback(Runnable r){
		endOnceCallback = r;
		return this;
	}
	...
	private void configureEmitter(View emitter, int gravity) {
		if(null==emitter){
			mEmitterXMin = 360 - 128;
			mEmitterXMax = 360 + 128;

			mEmitterYMin = 760 - 128;
			mEmitterYMax = 760 + 128;
			return;
		}
		// It works with an emision range
		int[] location = new int[2];
		emitter.getLocationOnScreen(location);

		mEmitterXMin = 360 - (emitter.getWidth()>>1);
		mEmitterXMax = 360 + (emitter.getWidth()>>1);

		mEmitterYMin = 760 - (emitter.getHeight()>>1);
		mEmitterYMax = 760 + (emitter.getHeight()>>1);
	}
